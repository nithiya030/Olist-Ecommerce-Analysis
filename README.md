...
Static PDF export of the five-page Power BI dashboard for users who do not have Power BI installed.

> The Power BI `.pbix` source file is not included in this repository due to GitHub file-size limitations.
...
**[View Interactive Power BI Dashboard](https://app.powerbi.com/groups/me/reports/e6d088b2-b49d-4811-a8d7-c6082dc0a67d?pbi_source=desktop)**

The interactive dashboard allows users to explore the analysis using filters, slicers, and interactive visualizations.

### 📄 Dashboard PDF

**[View / Download Dashboard PDF](olist_ecommerce_dashboard.pdf)**
...
The `.pbix` source file is not included in the repository because of its file size.

The interactive dashboard can be accessed through the **Power BI link provided above**.

A static PDF version of the dashboard is also included in the repository.

---

## Step 5 — Refresh the Data Model

To reproduce the Power BI analysis independently:

1. Run the Python cleaning notebook.
2. Generate the cleaned CSV files.
3. Open the Power BI project created from the cleaned data.
4. Update the data source paths.
5. Open Power Query and verify the file paths.
6. Confirm the required tables are loaded.
7. Apply any pending transformations.
8. Close & Apply.
9. Refresh the Power BI data model.
